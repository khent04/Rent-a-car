...
class Vendor(User):
    ...
    # logo = blob TBA
    credential = ndb.KeyProperty(kind='Certificate', required=False,
                                 repeated=True, indexed=False)
    # the vendor may upload file for aproval
    votes = ndb.KeyProperty(kind='User', required=False, repeated=True,
                            indexed=False)
    # came from the renter who already have experience, new users has no
    # capability of doing so to avoid hoax credibility
    credibility = ndb.IntegerProperty(required=False, indexed=False)
